...
function 김비서나와라() {
  ...
  // 3-9. 중앙이동을 위한 top,left 값 변경
  나변수.style.top = "50%";
  나변수.style.left = "50%";

  // 3-10. 트랜스폼 변경 : 중앙이동, 360도 회전, 스케일변경
...
function 맘대로해라(헐, 헉스) {
  ...
  /**************************************************************
   [ JS DOM의  특별한 선택 매서드 2가지] 
    1. querySelectorAll(CSS 선택자) : 여러개 선택
     -> 2번 방식은 HTMLCollection을 반환한다!
     -> 따라서 length와 item(순번)/[순번]을 사용함!
   **************************************************************/


 /*  document.getElementsByClassName("박스야")
    .item(0).getElementsByClassName("원이야").item(헐); */
  ...
  // 3-3. 유튜브 동영상 넣기
  아파트.innerHTML = `<iframe src="https://www.youtube.com/embed/8Ebqe2Dbzls?autoplay=1; " 
  allow="autoplay"></iframe>`;
  // 3-4. 아이프레임 디자인 넣기: 선택된 아파트 하위 iframe
  var 뮤비 = 아파트.querySelector('iframe');
  // cssText속성 : 한꺼번에 문자열로 CSS 넣을때 사용
