...
function makeList(){
    ...
    // for in문으로 돌린것 처럼 키 값을 순회하여 셋팅가능함!!!

    newVal.forEach(v=>{ // X - 배열값(객체의 키)
...
/******************************************** 
     [ 객체를 위한 for in문 ]
      - 구문 :
      for(변수 in 객체){코드}
      -> 객체를 순회하면 변수에 담는 것을 속성(key)이다!

      ((같지만 다르게 표현하면))
      for(ket in Object){코드}
      -> 객체의 구성은 속성(key)과 값(value)이다!
      -> 객체의 구성 - {key:value,key:value,...}

      (2) 작동의 원리:
      - 객체의 개수만큼 순서대로 속성 즉, key를 가져옴!
